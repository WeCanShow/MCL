...
def get_train_loader(opt):
    ...
    if (opt.dataset == 'CIFAR10'):
        ...
        trainset = ImageFolder('data/Imagenet20/train',transform=transforms.Resize((224, 224)))
...
def get_test_loader(opt):
    ...
    if (opt.dataset == 'CIFAR10'):
        ...
        testset = ImageFolder('./data/Imagenet20/val',transform=tmp)
    elif opt.dataset == 'gtsrb':
...
class DatasetBD(Dataset):
    def __init__(self, opt, full_dataset, inject_portion, transform=None, mode="train", device=torch.device("cuda"), distance=1):
        self.dataset = self.addTrigger(full_dataset, opt.target_label, inject_portion, mode, distance, opt.trig_w, opt.trig_h, opt.trigger_type, opt.target_type, opt.dataset)
        self.device = device
        self.transform = transform
    ...
    def addTrigger(self, dataset, target_label, inject_portion, mode, distance, trig_w, trig_h, trigger_type, target_type, t_dataset=None):
        ...
        for i in tqdm(range(len(dataset))):
            ...
            if target_type == 'all2one':
                ...
                if mode == 'train':
                    ...
                    if i in self.perm:
                        # select trigger
                        img = self.selectTrigger(img, width, height, distance, trig_w, trig_h, trigger_type, t_dataset, idx=i)
            ...
            # clean label attack
                ...
                if mode == 'train':
                    ...
                    width = img.shape[0]
                    height = img.shape[1]
                    if random.random() < inject_portion*8:
                    #if i in perm:
                        if data[1] == target_label:
                            self.perm.append(i)
                            img = self.selectTrigger(img, width, height, distance, trig_w, trig_h, trigger_type, t_dataset, idx=i)
    ...
    def selectTrigger(self, img, width, height, distance, trig_w, trig_h, triggerType, t_dataset=None, idx=0):
        ...
        assert triggerType in ['squareTrigger', 'gridTrigger',
                               'signalTrigger', 'trojanTrigger', 'blendTrigger', 'centerTrigger']
        ...
        # bd_img = Image.fromarray(img)
        # bd_img.save('images/' + t_dataset + '/blend/' + str(idx) + '.png')
    ...
    def _blendTrigger(self, img, width, height, distance, trig_w, trig_h, t_dataset):
        ...
        if t_dataset == 'imagenet':
            poison_img = Image.fromarray(poison_img)
            trigger_ptn = Image.open('trigger/hello_kitty.png').convert("RGB")
            trigger_ptn = trigger_ptn.resize(poison_img.size)
            poison_img = Image.blend(poison_img, trigger_ptn, alpha)
